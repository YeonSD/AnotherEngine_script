class Battle
  ...
  def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, showMessages = true)
    ...
    exp = (exp * 1.0).floor if Settings::MORE_EXP_FROM_TRAINER_POKEMON && trainerBattle?
    ...
    loop do   # For each level gained in turn...
      ...
      moveList.each { |m| pbLearnMove(idxParty, m[1]) if m[0] == curLevel }
			...
			# ★★★★★ [추가된 부분] ★★★★★
      # 레벨업한 포켓몬을 대상으로 '특성 각성' 함수를 실행합니다.
			pbLearnRandomAbilityOnLevelUp(pkmn)
      pbLearnPassiveAbilityOnLevelUp(pkmn)
    end
  end
