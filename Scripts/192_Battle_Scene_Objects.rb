...
#    pbSetSmallFont(@hpNumbers.bitmap)
  ...
  # 타입 띄우기
  def draw_type_icons
    # echoln "Switch 279 is #{$game_switches[279]}"
    return if !$game_switches[279]
    
    path = pbResolveBitmap("Graphics/UI/types_short")
    return if !path
    type_bitmap = AnimatedBitmap.new(path)
    
    begin
      current_types = @battler.types 
      return if current_types.empty?
      
      base_x = @battler.index.even? ? 220 : 210
      base_y = 0
    
      current_types.reverse.each_with_index do |type, i|
        type_number = GameData::Type.get(type).icon_position
        type_rect = Rect.new(0, type_number * 28, 28, 28)
        target_rect = Rect.new(base_x - (i * 17), base_y, 15, 15)
        
        self.bitmap.stretch_blt(target_rect, type_bitmap.bitmap, type_rect)
      end
    ensure
      type_bitmap.dispose
    end
