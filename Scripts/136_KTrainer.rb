    ...
    def to_trainer
      ...
      @pokemon.each do |pkmn_data|
        ...
        pkmn.item = pkmn_data[:item]
				switchcheck1 = $player.random_move_switch
				switchcheck2 = $player.random_exmove_switch
				$player.random_move_switch = false if switchcheck1
				$player.random_exmove_switch = false if switchcheck2
        if pkmn_data[:moves] && pkmn_data[:moves].length > 0
          pkmn_data[:moves].each { |move| pkmn.learn_move(move) }
        else
					pkmn.reset_moves
				end
				$player.random_move_switch = switchcheck1
				$player.random_exmove_switch = switchcheck2
        ...
        GameData::Stat.each_main do |s|
          ...
          if pkmn_data[:ev]
            ...
            pkmn.ev[s.id] = [pkmn_data[:level] * 3 / 2, Settings.ev_limit / 6].min
          end
        end
