  ...
  def choose_wild_pokemon(enc_type, chance_rolls = 1)
    ...
    enc_list.each do |enc|
      ...
      break
		...
		if $player.random_pokemon_switch #랜덤 포켓몬
			new_species = :NECROZMA_3
			encounter[1] = give_random_pokemon(new_species)
		end
...
def pbGenerateWildPokemon(species, level, isRoamer = false)
  ...
  # 방금 태어난 야생 포켓몬에게 메시지 없는 랜덤 특성 부여
	give_random_ability_wild(genwildpoke)
	giverandom(genwildpoke)
