...
class MoveRelearnerScreen
  ...
  def pbStartScreen(pkmn)
    moves = ($random_move_switch) ? pbGetRandomMovesForPkmn(pkmn) : pbGetRelearnableMoves(pkmn)
...
def pbRelearnMoveScreen(pkmn)
  current_pkmn = pkmn
  overall_retval = false
  
  # 에센셜 버전에 따라 플레이어 객체가 $player인지 $Trainer인지 자동 판별
  player_obj = defined?($player) ? $player : $Trainer 
  
  loop do
    # 1. 한 포켓몬에게 기술을 계속 가르치는 루프
    loop do
      retval = false
      ...
      pbFadeOutIn do
        ...
        retval = screen.pbStartScreen(current_pkmn)
      end
      
      overall_retval = true if retval
      break if !retval # 뒤로가기를 누르면 이 포켓몬은 끝!
    end
    
    # 2. 다음 포켓몬을 고를 수 있게 파티 화면 호출
    pbChoosePokemon(1, 2)
    break if $game_variables[1] < 0 # 파티창에서 취소 누르면 완전 종료
    
    # 선택한 포켓몬으로 교체
    current_pkmn = player_obj.party[$game_variables[1]] 
    
    # 알 예외 처리
    while current_pkmn.egg?
      pbMessage(_INTL("알은 기술을 떠올릴 수 없습니다."))
      pbChoosePokemon(1, 2)
