      ...
      when 0   # Set EVs
        ...
        loop do
          ...
          cmd2 = screen.pbShowCommands(_INTL("Change which EV?\nTotal: {1}/{2} ({3}%)",
                                             totalev, Settings.ev_limit,
                                             100 * totalev / Settings.ev_limit), evcommands, cmd2)
          break if cmd2 < 0
          if cmd2 < ev_id.length
            params = ChooseNumberParams.new
            upperLimit = 0
            GameData::Stat.each_main { |s| upperLimit += pkmn.ev[s.id] if s.id != ev_id[cmd2] }
            upperLimit = Settings.ev_limit - upperLimit
            ...
            evTotalTarget = Settings.ev_limit
            if cmd2 == evcommands.length - 2   # Randomize all (not max)
              evTotalTarget = rand(Settings.ev_limit)
            end
