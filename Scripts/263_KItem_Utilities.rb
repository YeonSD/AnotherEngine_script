  ...
  if old_level > new_level
    ...
    movelist.each do |i|
      ...
      pbLearnMove(pkmn, i[1], true) { scene.pbUpdate }
		...
		pbLearnRandomAbilityOnLevelUp(pkmn)#특성 습득
		pbLearnPassiveAbilityOnLevelUp(pkmn)#패시브 습득
    # Check for evolution
    new_species = pkmn.check_evolution_on_level_up
		
		if $player.random_evo_switch != 0 && pkmn.item != :EVERSTONE && $player.random_evo_switch#랜덤 진화
			evo_level = $player.random_evo_level || 1
			if new_species
				new_species = give_random_pokemon(pkmn)
			elsif new_level/evo_level > old_level/evo_level
				new_species = give_random_pokemon(pkmn)
			end
			new_species = nil if !$player.random_evo_allow && pkmn.species_data.get_evolutions(true).length == 0
		end
...
def pbJustRaiseEffortValues(pkmn, stat, evGain)
  ...
  evGain = evGain.clamp(0, Settings.ev_limit - evTotal)
  if evGain > 0
    pkmn.ev[stat] += evGain
    pkmn.calc_stats
  end
...
def pbMaxUsesOfEVRaisingItem(stat, amt_per_use, pkmn, no_ev_cap = false)
  ...
  amt_can_gain = [amt_can_gain, Settings.ev_limit - ev_total].min
