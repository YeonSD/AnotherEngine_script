...
Battle::ItemEffects::DamageCalcFromTarget.add(:SOFTTAIL,
  proc { |item, user, target, move, mults, power, type|
    next unless target.isSpecies?(:EEVEE)
...
Battle::ItemEffects::DamageCalcFromUser.add(:PIKANIUMZ,
  proc { |item, user, target, move, mults, power, type|
    if user.isSpecies?(:PIKACHU) && user.form == 18
      mults[:attack_multiplier] *= 2
    end
  }
)

Battle::ItemEffects::DamageCalcFromUser.copy(:PIKANIUMZ, :PIKASHUNIUMZ)

Battle::ItemEffects::DamageCalcFromUser.add(:MAXMUSHROOM,
  proc { |item, user, target, move, mults, power, type|
...
Battle::ItemEffects::DamageCalcFromTarget.add(:MAXMUSHROOM,
  proc { |item, user, target, move, mults, power, type|
    if target.isSpecies?(:EEVEE) && target.form != 0
      mults[:defense_multiplier] *= 2
    end
  }
)

Battle::ItemEffects::DamageCalcFromUser.copy(:MAXMUSHROOM, :TERAPIECE)

Battle::ItemEffects::DamageCalcFromTarget.copy(:MAXMUSHROOM, :TERAPIECE)
...
Battle::ItemEffects::WeatherExtender.add(:DAMPROCK,
  proc { |item, weather, duration, battler, battle|
    if $game_variables[129] == 1
      next -1 if weather == :Rain
    else
