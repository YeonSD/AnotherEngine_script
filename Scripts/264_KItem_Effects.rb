...
ItemHandlers::UseFromBag.add(:REDCONTROLLER, proc { |item|
  next 2
})
...
ItemHandlers::UseInField.add(:REPEL, proc { |item|
  steps = 100
  steps = $game_variables[149] if $game_variables[149] > 0
  next pbRepel(item, steps)
})

ItemHandlers::UseInField.add(:SUPERREPEL, proc { |item|
  steps = 200
  steps = $game_variables[149] * 2 if $game_variables[149] > 0
  next pbRepel(item, steps)
})

ItemHandlers::UseInField.add(:MAXREPEL, proc { |item|
  steps = 250
  steps = $game_variables[149] * 3 if $game_variables[149] > 0
  next pbRepel(item, steps)
})
...
ItemHandlers::UseInField.add(:REDCONTROLLER, proc { |item|
  
  
  pbWait(0.5)
  $game_player.animation_id = 14

  pbMessage("\\xn[\\c[1]\\pn]\\c[1](레드 방에 있는 전자기기들이 연결됐다!)")
...
loop do
  ...
  choice = pbMessage("\\xn[\\c[1]\\pn]\\c[1](원격으로 무엇을 실행할까?)", commands, commands.size)
  break if choice.nil? || choice >= commands.size - 1

  case choice
  when 0 then pbCustomB
  when 1 then pbCustomA
  when 2 then pbCustomC
  when 3 then pbPokeCenterPC
  when 4 then pbCallMapEvent(58, 13, [75, 71, 69, 70, 150])
  when 5 then pbCallMapEvent(158, 5, [117])
  when 6 then pbpokesys
  when 7 then pbToggleRandomSettings
  when 8 then pbAceSelectionMenu
  when 9 then pbExportFullRandomDataHtml
  end
end
  next true
})
...
ItemHandlers::UseOnPokemon.addIf(:evolution_stones,
  ...
  proc { |item, qty, pkmn, scene|
    ...
    if newspecies
			if $player.random_evo_switch != 0 && pkmn.hasItem? != :EVERSTONE && $player.random_evo_switch#랜덤 진화
				level = pkmn.level
				new_species = give_random_pokemon(pkmn)
				pkmn.level = level
			end
...
ItemHandlers::UseOnPokemon.add(:ABILITYCAPSULE, proc { |item, qty, pkmn, scene|
	if scene.pbConfirm(_INTL("Do you want to change {1}'s Ability?", pkmn.name))
		if pkmn.random_ability_list.length > 0
			pkmn.random_ability_list = []
			pkmn.ability_index = 0
			newabilname = GameData::Ability.get(pkmn.ability).name
			scene.pbRefresh
...
ItemHandlers::UseOnPokemon.add(:DNASPLICERS, proc { |item, qty, pkmn, scene|
	if !pkmn.isSpecies?(:KYUREM) || !pkmn.fused.nil?
		if !$player.fusion_switch	
			scene.pbDisplay(_INTL("유전자쐐기가 반응하지 않는다.")) 
			next false
		end
	elsif pkmn.fainted?
		scene.pbDisplay(_INTL("This can't be used on the fainted Pokémon."))
  ...
  if pkmn == other_pkmn
	...
	if pkmn.isSpecies?(:KYUREM) && !other_pkmn.isSpecies?(:RESHIRAM) && !other_pkmn.isSpecies?(:ZEKROM) || !pkmn.isSpecies?(:KYUREM)
		pokemonFusion(pkmn, other_pkmn)
		$player.remove_pokemon_at_index(chosen)
		scene.pbHardRefresh
		scene.pbDisplay(_INTL("\\j[{1},과,와] \\j[{2},과,와] 융합하였다.", pkmn.name, other_pkmn.name))
		next true
	end
	if !other_pkmn.isSpecies?(:RESHIRAM) && !other_pkmn.isSpecies?(:ZEKROM)
