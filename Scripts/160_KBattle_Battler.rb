class Battle::Battler
  ...
  #  return self.ability == check_ability
	...
	def hasActiveAbility?(check_ability, ignore_fainted = false)
		# 1. '위액' 등 특성을 무효화하는 효과가 있는지 먼저 확인합니다.
    return false if !abilityActive?(ignore_fainted, check_ability)
    
    # 2. 포켓몬의 기본 특성이 찾고 있는 특성과 일치하는지 확인합니다.
    has_main_ability = false
    if check_ability.is_a?(Array)
      has_main_ability = check_ability.include?(@ability_id)
    else
      has_main_ability = (@ability_id == check_ability)
    end
    return true if has_main_ability

    # 3. 기본 특성이 아니라면, 이제 패시브 특성이 있는지 확인합니다.
    # 포켓몬 객체나 패시브 변수가 존재하지 않으면 즉시 종료합니다.
    return false if !self.pokemon
    passive = self.pokemon.passive_ability
    return false if !passive
    
    # 4. 패시브 특성이 찾고 있는 특성과 일치하는지 확인합니다.
    has_passive_ability = false
    if check_ability.is_a?(Array)
      has_passive_ability = check_ability.include?(passive)
    else
      has_passive_ability = (passive == check_ability)
    end
    return true if has_passive_ability

    # 5. 기본 특성도, 패시브도 아니라면 최종적으로 false를 반환합니다.
    return false
  end
