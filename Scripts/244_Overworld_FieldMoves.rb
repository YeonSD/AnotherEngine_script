...
def pbCut
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_CUT, false) || (!has_cut_tm && !movefinder)
...
def pbHeadbuttEffect(event = nil)
  ...
  when 1
    ...
    chance = 1
    chance = 8 if a == b
    chance = 5 if (a > b && (a - b).abs < 5) || (a < b && (a - b).abs > 5)
    max_rand = 10
  end

  if rand(max_rand) >= chance
...
def pbRockSmash
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH, false) || (!has_rocksmash_tm && !movefinder)
...
def pbStrength
  ...
  move = :STRENGTH
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH, false) || (!movefinder)
