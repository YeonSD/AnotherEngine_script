  ...
  def battler=(value)
    ...
    if @battler
      @battler.moves.each do |move|
        # 1. 버튼 그래픽 정보를 미리 계산 (이전과 동일)
        type_icon_pos = GameData::Type.get(move.display_type(@battler)).icon_position
        @move_button_data.push(type_icon_pos)
        
        # 2. 표시될 위력을 미리 계산 (지연 현상 해결의 핵심)
        # 여기서 시간 걸리는 계산을 단 한 번만 수행합니다.
        display_power = PassivePowerCalculator.get_display_power(move, @battler)
        @move_power_cache.push(display_power)
      end
    end
    refresh
    refreshButtonNames
