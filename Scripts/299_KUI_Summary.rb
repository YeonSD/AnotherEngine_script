...
class PokemonSummary_Scene
  ...
  def drawPageTwo
    ...
    if @pokemon.timeReceived
      ...
      month = pbGetMonthName(@pokemon.timeReceived.mon)
      year  = @pokemon.timeReceived.year
      memo += black_text_tag + _INTL("{3}년{2}{1}일", date, month, year) + "\n"
    end
    ...
    memo += black_text_tag + mettext + "\n" if mettext && mettext != ""
		...
		if @pokemon.fusion && $player.fusion_switch
			i=0
			if @pokemon.fusion.size > 4
				3.times do
					memo += black_text_tag + _INTL("\\j[{1},과,와] ",@pokemon.fusion[i])
					i+=1
				end
				memo += black_text_tag + _INTL("추가로 {1}체와 융합",@pokemon.fusion.size-3)+ "\n"
			else
				@pokemon.fusion.size.times do
					memo += black_text_tag + _INTL("\\j[{1},과,와] ",@pokemon.fusion[i])
					i+=1
				end
				memo += black_text_tag + _INTL("융합")+ "\n"
			end
		else
    ...
    # --- 패시브 정보 표시 ---
    if @pokemon.passive_ability
      passive_name = GameData::Ability.get(@pokemon.passive_ability).name
      passive_desc = GameData::Ability.get(@pokemon.passive_ability).description

      # "패시브" 제목
      memo += black_text_tag + _INTL("패시브 ") + passive_name +"\n"
      memo += black_text_tag + passive_desc
		end
