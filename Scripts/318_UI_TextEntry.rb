...
class PokemonEntryScene2
  @@Characters = [
    [("ㅃㅉㄸㄲㅆ!?~ㅒㅖ ♂♀" + "ㅂㅈㄷㄱㅅㅛㅕㅑㅐㅔ   " + "ㅁㄴㅇㄹㅎㅗㅓㅏㅣ/   " + "ㅋㅌㅊㅍ ㅠㅜㅡ,.   " + "0123456789   ").scan(/./), _INTL("hangeul")],
  ...
  def pbStartScene(helptext, minlength, maxlength, initialText, subject = 0, pokemon = nil)
    ...
    @viewport.z = 99999
    @helptext = helptext
    @helper = CharacterEntryHelper.new(initialText, maxlength)
    # Create bitmaps
    @bitmaps = []
    @@Characters.length.times do |i|
      ...
      COLUMNS.times do |y|
        ROWS.times do |x|
          pos = (y * ROWS) + x
          textPos.push([@@Characters[i][0][pos], 44 + (x * 32), 26 + (y * 38), :center,
    ...
    underline_bitmap = Bitmap.new(24, 6)  # 이름 입력란
  ...
  def pbUpdate
    ...
    cursorpos = @helper.cursor.clamp(0, @maxlength)
    @maxlength.times do |i|
    ...
    loop do
      ...
      if Input.trigger?(Input::SPECIAL)
        ...
        when MODE4
          ...
          @helper.insert(chset[charpos]) ? pbPlayCursorSE : pbPlayBuzzerSE

          if @helper.isTextFull?
            @cursorpos = OK
            @sprites["cursor"].setCursorPos(@cursorpos)
          end
          pbUpdateOverlay
