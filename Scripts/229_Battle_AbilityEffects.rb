...
Battle::AbilityEffects::PriorityChange.add(:GALEWINGS,
  proc { |ability, battler, move, pri|
   if $game_switches && $game_switches[273]
...
Battle::AbilityEffects::DamageCalcFromUser.copy(:AERILATE, :GALVANIZE, :NORMALIZE, :PIXILATE, :REFRIGERATE, :IRONSKIN)
...
Battle::AbilityEffects::OnSwitchIn.add(:SLOWSTART,
  proc { |ability, battler, battle, switch_in|
    ...
    if Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("\\j[{1},은,는] 기운이 나지 않는다!", battler.pbThis))
    else
      battle.pbDisplay(_INTL("\\j[{1},은,는] \\j[{2},으로,로] 인해 기운이 나지 않는다!",
         battler.pbThis, battler.abilityName))
    end
    battle.pbHideAbilitySplash(battler)
  }
