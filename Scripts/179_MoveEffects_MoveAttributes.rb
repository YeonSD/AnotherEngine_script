...
#===============================================================================
# Deals damage to the target and sets Reflect on the user's side
#===============================================================================
class Battle::Move::DamageAndReflect < Battle::Move
  def canSnatch?; return true; end   # 기존 Reflect 클래스와 동일
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    user.pbOwnSide.effects[PBEffects::Reflect] = 5
    user.pbOwnSide.effects[PBEffects::Reflect] = 8 if user.hasActiveItem?(:LIGHTCLAY)
    @battle.pbDisplay(_INTL("리플렉터가 {1}의 방어를 올려줬다!", user.pbTeam(true)))
  end
end
...
# Deals damage to the target and sets Reflect on the user's side
#===============================================================================
class Battle::Move::DamageAndLightScreen < Battle::Move
  def canSnatch?; return true; end   # 기존 Reflect 클래스와 동일
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    user.pbOwnSide.effects[PBEffects::LightScreen] = 5
    user.pbOwnSide.effects[PBEffects::LightScreen] = 8 if user.hasActiveItem?(:LIGHTCLAY)
    @battle.pbDisplay(_INTL("빛의장막이 {1}의 특수방어를 올려줬다!", user.pbTeam(true)))
  end
end

#===============================================================================
# Deals damage and plants Leech Seed on the target (if possible)
#===============================================================================
class Battle::Move::DamageAndLeechSeed < Battle::Move
  def pbEffectAgainstTarget(user, target)
    # === 1. 기본 데미지 ===
    super if defined?(super)
    ...
    return if target.damageState.substitute   # 분신 보호 중이면 무효
...
#===============================================================================
# Deals damage, then resets all stat stages for all battlers to 0. (Damage + Haze)
#===============================================================================
class Battle::Move::DamageAndHaze < Battle::Move
  def pbEffectAgainstTarget(user, target)
    # === 1. 기본 데미지 처리 ===
    super if defined?(super)
  end
  ...
  def pbEffectGeneral(user)
    ...
    # === 3. 연출 ===
    @battle.pbAnimation(:HAZE, user, nil)
    @battle.pbDisplay(_INTL("All stat changes were eliminated!"))
  end
end
#============================================================================
class Battle::Move::DamageAndRaiseEvasion < Battle::Move
  def pbEffectAgainstTarget(user, target)
    # 1. 기본 데미지 처리
    super if defined?(super)
...
#===============================================================================
# Deals damage to the target, then cures all status conditions on the user's side.
# (ex: Custom move - Damage + Aromatherapy)
#===============================================================================
class Battle::Move::DamageAndCureUserPartyStatus < Battle::Move
  def canSnatch?;          return true; end
  def worksWithNoTargets?; return false; end

  # 기본 데미지 처리
  def pbEffectAgainstTarget(user, target)
    super if defined?(super) # 일반 데미지 계산
  end

  # 데미지를 준 후, 아군 전체 상태이상 회복
  def pbEffectGeneral(user)
    side_has_status = false
    @battle.allSameSideBattlers(user).each do |b|
      next if b.status == :NONE
      b.pbCureStatus(false)
      @battle.pbDisplay(_INTL("{1}의 상태이상이 회복되었다!", b.pbThis))
      side_has_status = true
    end
    ...
    party.each_with_index do |pkmn, i|
      ...
      next if @battle.pbFindBattler(i, user)
      pkmn.status = :NONE
      pkmn.statusCount = 0
      @battle.pbDisplay(_INTL("{1}의 상태이상이 회복되었다!", pkmn.name))
      side_has_status = true
    end
    ...
    if !side_has_status
      ...
      @battle.pbCommonAnimation("Move_HEALBELL", user)
      @battle.pbDisplay(_INTL("맑은 종소리가 울려 퍼졌다!"))
    end
  end
end