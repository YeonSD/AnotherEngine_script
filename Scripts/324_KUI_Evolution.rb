...
class PokemonEvolutionScene
  ...
  def pbEvolution(cancancel = true)
		if $player.random_evo_switch != 0
			cancancel = false if $player.random_evo_switch == 2
			level = @pokemon.level
		end
    pbBGMStop
    ...
    if canceled
      ...
      pbEvolutionSuccess
		...
		if $player.random_evo_switch != 0
			@pokemon.level = level if @pokemon.level != level 
		end
  end
...
#kj
  def pbEvolutionSuccess
    ...
    @pokemon.calc_stats
		...
		if $player.random_ability_switch != 0 && !$player.random_ability_switch
			# 진화 후 모든 폼에 특성 부여 후 기존에 사용하던 특성 회수
			level_up_pool = (GROUP_A_ABILITIES + GROUP_C_ABILITIES)
			level_up_pool += GameData::Ability.keys if rand < 0.65
			level_up_pool.reject! { |move| FORM_CHANGE_ABILITIES.include?(move) }
			@pokemon.random_ability_list = []
			GameData::Species.each do |sp|
				next if sp.species != @pokemon.species
				loop do
					new_ability = level_up_pool.sample
					break if new_ability != @pokemon.ability_id
				end
				@pokemon.random_ability_list.push(new_ability)
			end
			new_ability = @pokemon.random_ability_list[@pokemon.form]
			old_ability_name = GameData::Ability.get(old_ability).name
			...
			if pbConfirmMessage(_INTL("기존의 특성 「{1}」을(를) 잊고 새로운 특성 「{2}」을(를) 배우시겠습니까?", old_ability_name, new_ability_name))
				pbMessage(_INTL("\\j[{1},의,의] 특성은 「{2}」이 되었다!", @pokemon.name, new_ability_name))
				@pokemon.ability = new_ability
			else
				@pokemon.random_ability_list[@pokemon.form] = old_ability
				@pokemon.ability = old_ability
			end
		end
		giverandom(@pokemon)
    ...
    end # ★★★★★ [수정된 부분] if 문을 닫아주는 end를 추가했습니다. ★★★★★
