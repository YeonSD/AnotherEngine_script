...
MenuHandlers.add(:party_menu, :relearn_move, {
  ...
  "condition" => proc { |screen, party, party_idx|
    next !party[party_idx].egg? && $game_variables[144] == 1 && $game_switches[4] && !$game_switches[10]
  },
  "effect"    => proc { |screen, party, party_idx|
    # 1️⃣ 변수 34, 35 먼저 초기화
    pbSet(34, 0)
    ...
    if moves.empty?
      screen.pbDisplay(_INTL("{1}은(는) 떠올릴 기술이 없습니다.", pkmn.name))
      next
    end
...
MenuHandlers.add(:party_menu, :relearn_move2, {
  "name"      => _INTL("기술 배우기"),
  "order"     => 70,
  "condition" => proc { |screen, party, party_idx|
    next !party[party_idx].egg? && $game_variables[144] == 1 && $game_switches[5] && $game_switches[10]
  },
  "effect"    => proc { |screen, party, party_idx|
    # 1️⃣ 변수 34, 35 먼저 초기화
    pbSet(34, 2)
