...
#===============================================================================
class Pokemon
  # class Pokemon 바로 아래에 추가
  attr_accessor :ability_override
  # 현재 폼 보관용
  attr_accessor :form_record
  attr_accessor :ability_record
  # 랜덤 타입 보관용
  attr_accessor :random_type
  # 랜덤 특성 보관용
  attr_accessor :random_ability_list
  # 랜덤 기술 보관용
  attr_accessor :random_move_list
  # 랜덤 기술 보관용
  attr_accessor :random_exmove_list
  # 랜덤 종족값 보관용
  attr_accessor :random_stats
  # 융합 보관
  attr_accessor :fusion
  attr_accessor :fusion_stat
  attr_accessor :fusion_move
  ...
  def types
		self.random_ability_list ||= []
		if self.random_ability_list&.[](self.form) && $player.random_ability_switch# 랜덤 특성을 받았을 경우에만 호출
			if !$game_temp.in_battle || self.form_record != self.form && $game_temp.in_battle
				self.ability = self.random_ability_list[self.form]
				self.form_record = self.form
			end
		end
		self.ability_record = self.ability if self.ability_record != self.ability
		if @random_type.is_a?(Hash) && $player.random_type_switch# 랜덤 타입을 받았을 경우에만 호출
			if @random_type&.[](self.form)
				current_form_id = self.form
				stored_types = @random_type[current_form_id]#폼 상태 보고 뽑아오기
				if stored_types.is_a?(Array) && stored_types.length > 0
					return stored_types.compact.clone
				end
			end
		end
		#기본 타입을 반환
  ...
  # --- ▼▼▼ 이 부분을 아래 코드로 통째로 교체해주세요 ▼▼▼ ---

  # [1. 이것이 지워졌던 '원본' ability_id 함수입니다. 반드시 필요합니다.]
  def ability_id
    if !@ability
  ...
  # [2. 저희가 추가하는 '임시 통행증' 패시브 로직입니다.]
  alias _passive_original_ability_id ability_id
  def ability_id
    return @ability_override if @ability_override
    return _passive_original_ability_id
  end
  ...
  def getMoveList
		movelist = []
		if $player.random_move_switch#랜덤 기술 대신 호출
			movelist = getRandomMoveList(self)
		else
			movelist = species_data.moves
		end
		if self.fusion_move && $player.fusion_switch#랜덤 기술 대신 호출
			movelist += self.fusion_move
		end
		if $player.random_exmove_switch
			movelist += getRandomEXMoveList(self)
		end
		return movelist
  end
  ...
  def trigger_event_evolution(number)
    ...
    if new_species
			if $player.random_evo_switch != 0 && self.hasItem != :EVERSTONE && $player.random_evo_switch#랜덤 진화
				level = pkmn.level
				new_species = give_random_pokemon(self)
				pkmn.level = level
			end
  ...
  def baseStats
    ...
    GameData::Stat.each_main { |s| ret[s.id] = this_base_stats[s.id] }
		if @random_stats.is_a?(Hash) && $player.random_stats_switch# 랜덤 스텟을 받았을 경우에만 호출
			if @random_stats&.[](self.form)
				current_form_id = self.form
				stored_stats = @random_stats[current_form_id]#폼 상태 보고 뽑아오기
				if stored_stats.is_a?(Array) && stored_stats.length > 0
					ret[:HP] = stored_stats[0]
					ret[:ATTACK] = stored_stats[1]
					ret[:DEFENSE] = stored_stats[2]
					ret[:SPECIAL_ATTACK] = stored_stats[3]
					ret[:SPECIAL_DEFENSE] = stored_stats[4]
					ret[:SPEED] = stored_stats[5]
				end
			end
		end
		if self.fusion_stat && $player.fusion_switch# 융합 스텟을 받았을 경우에만 호출
			stored_stats = self.fusion_stat
			ret[:HP] += stored_stats[0]
			ret[:ATTACK] += stored_stats[1]
			ret[:DEFENSE] += stored_stats[2]
			ret[:SPECIAL_ATTACK] += stored_stats[3]
  ...
  def initialize(species, level, owner = $player, withMoves = true, recheck_form = true)
    ...
    @ability_index    = nil
    @ability          = nil
		@random_ability_list = [] #랜덤 특성들 저장소
		@random_type 			= nil #랜덤 타입
		@random_stats 			= nil #랜덤 타입
		@random_move_list = nil
		@random_exmove_list = nil
		@fusion 					= nil #융합 소재 저장소
		@fusion_move 					= nil #융합 소재 저장소
		@fusion_stat 			= nil #융합 소재 저장소
		@ability_record 	= nil #특성기록
    @nature           = nil
...
def give_random_ability(pokemon)
  ...
  special_exclusions = [
    ...
    :DRAGONSMAW, :UNSEENFIST
  ]
  all_abilities = GameData::Ability.keys
  valid_abilities = all_abilities - special_exclusions
  ...
  # 야생 포켓몬이 아닐 때만 메시지 표시
  if !is_wild
    new_ability_name = GameData::Ability.get(new_ability).name
    pbMessage(_INTL("{1}은/는 새로운 특성 「{2}」을/를 얻었다!", pokemon.name, new_ability_name))
  end
end
