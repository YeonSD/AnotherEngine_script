...
def pbFishing(hasEncounter, rodType = 1)
  ...
  loop do
    ...
    time = rand(2..3) if Settings.fishing_auto_hook()
    message = ""
    time.times { message += ".   " }
    ...
    if hasEncounter && rand(100) < biteChance
      ...
      #Settings.fishing_auto_hook?
      if Settings.fishing_auto_hook() || rand(100) < hookChance
        pbFishingEnd do
          pbMessageDisplay(msgWindow, _INTL("Landed a Pokémon!")) if !Settings.fishing_auto_hook() ##Settings.fishing_auto_hook
        end
        ret = true
        break
      end
